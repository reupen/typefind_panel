...
class progressive_search {
    ...
    void run()
    {
        ...
        if (!b_clear && m_string.get_length()) {
            ...
            // m_filter.get_size()));
            size_t the_focus = api->playlist_get_focus_item(active);
...
public:
    void init()
    {
        if (!m_running) {
            ...
            const auto count = api->playlist_get_item_count(active);
    ...
    bool on_key(WPARAM wp)
    {
        ...
        case VK_RETURN: {
            ...
            if (ctrl_down) {
                ...
                const size_t focus = api->playlist_get_focus_item(active);
        ...
        case VK_DOWN: {
            ...
            const auto focus = api->playlist_get_focus_item(active);
